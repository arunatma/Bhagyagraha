...
def run():
    ...
    print(f"\n{'='*70}")
    print("  B H A G Y A G R A H A   —   Horoscope")
    print(f"{'='*70}")
    print(f"  Name        : {name}")
    ...
    print(
        f"  Longitude   : {input_params['long_degs']:.4f} {input_params['long_dirn']}"
    )
    ...
    print(
        f"  Tamil Date  : {tamil_day}  {TAMIL_MONTH[tamil_month]}  {TAMIL_YEAR[tamil_year % 60]}"  # noqa: E501
    )
    ...
    # Planetary positions
    print(f"{'='*70}")
    print("  NIRAYANA LONGITUDES")
    print(f"{'='*70}")
    print(
        f"  {'Planet':<8}  {'Long':>8}  {'Nakshatra':<14}  Pada  {'Rasi':<12}  Navamsa"
    )
    print(f"  {'-'*65}")
    for i, nm in enumerate(GRAHA_NAMES):
        ...
        print(
            f"  {nm:<8}  {d:3d}-{m:02d}      {NAKSHATRA[nak]:<14}  {pada}     "
    ...
    # House cusps
    print(f"{'='*70}")
    print("  BHAVA CUSPS (House Positions)")
    print(f"{'='*70}")
    for i in range(12):
    ...
    print("  SHADBALA")
    print(f"{'='*70}")
    print(f"  {'Balam':<22}" + "".join(f"{ln}" for ln in PLANET_LABELS))
    print(f"  {'-'*70}")
    rows = [
        ("Sthana Bala", "sthana"),
        ("Kala Bala", "kala"),
    ...
    print(
        f"  {'Min Required':<22}" + "".join(f"{v:<8.1f}" for v in shad["min_required"])
    )
    print(
        f"  {'Relative Strength':<22}" + "".join(f"{v:<8.2f}" for v in shad["relative"])
    )
    print(f"  {'-'*70}")
    ...
    print("  MUTUAL DISPOSITION (NAVAMSA)")
    print(f"  {header}")
