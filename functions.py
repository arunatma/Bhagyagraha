...
def get_years_elapsed(ist_time, epoch_time):
    ...
    years_elapsed = (
        time_elapsed.days + time_elapsed.seconds / cn.seconds_in_day
    ) / cn.solar_days_in_year
...
def get_precession_degs(years_elapsed):
    ...
    """
    precession_movement = (
        cn.precession_per_year_in_sec * years_elapsed
    ) / cn.seconds_in_degree
...
def get_equation_of_centre(e, mean_anomaly_degs):
    ...
    mandaphalam_secs = (
        ...
        * (
            ...
            + math.pow(e, 3) / 12.0 * (13.0 * math.sin(3.0 * ma) - 3.0 * math.sin(ma))
        )
        + 0.5
    )
...
def get_helio_centric_vel(mean_anomaly_degs):
    ...
    helio_vel_secs = (
        ...
        * (
            ...
            - e * e * e * (13.0 * math.cos(3.0 * ma) - math.cos(ma)) / 4.0
        )
        + 0.5
    )
...
def get_hour_angle(latitude_degs, trop_long_sun_degs):
    ...
    ltfi_rads = math.atan(
        abs(
            math.sqrt(
                1.0
                - math.sin(omega) * math.sin(omega) * math.sin(tlsr) * math.sin(tlsr)
            )
    ...
    hour_angle_rads = math.acos(
        (-1.0 * math.tan(lat_rads) * math.sin(omega) * math.sin(tlsr))
        / math.sqrt(
            1.0 - math.sin(omega) * math.sin(omega) * math.sin(tlsr) * math.sin(tlsr)
        )
    )
...
def get_pranam(trop_long_sun_degs):
    ...
    pranam_rads = abs(
        math.atan(
            (2.0 * math.tan(tlsr) * math.sin(omega / 2.0) * math.sin(omega / 2.0))
...
def get_net_correction(
    charam_degs, mandaphalam_secs, pranam_degs, longitude_degs, dirn
):
    """
...
def sun_corrected_long(true_long_sun_degs, net_corr_min, helio_vel_degs):
    ...
    corrected_true_long = find_sum_degs(
        [true_long_sun_degs, net_corr_day * helio_vel_degs]
    )
...
def get_sun_params(input_params):
    ...
    apse_posn_sun_degs = get_apse_position_degs(
        cn.apse_position_at_epoch, cn.apse_movement_per_year_in_sec, years_since_epoch
    )
    mean_anom_sun_degs = get_mean_anomaly_degs(apse_posn_sun_degs, mean_long_sun_degs)
    mandaphalam_secs = get_equation_of_centre(cn.earth_eccentricity, mean_anom_sun_degs)

    true_long_sun_degs = get_true_longitude(mean_long_sun_degs, mandaphalam_secs)
    trop_long_sun_degs = get_trop_longitude_sun(true_long_sun_degs, precession_degs)

    helio_vel_degs = get_helio_centric_vel(mean_anom_sun_degs)
    radius_vect_sun = get_radius_vector(
        apse_posn_sun_degs, trop_long_sun_degs, cn.earth_eccentricity, cn.earth_lsma
    )
    ...
    net_corr_min = get_net_correction(
        charam_degs, mandaphalam_secs, pranam_degs, longitude_degs, dirn
    )
    true_long_sun_degs = sun_corrected_long(
        true_long_sun_degs, net_corr_min, helio_vel_degs
    )

    sunrise_dt, sunset_dt = get_sun_rise_set(
        in_datetime,
        trop_long_sun_degs,
        mandaphalam_secs,
        mean_anom_sun_degs,
...
def calc_tamil_date(input_params, fine_tuning=True):
    ...
    tamil_year_num = int(
        (original_time.year - cn.tamil_year_base) % cn.total_tamil_years
    )
...
def get_local_time_correction(localtm):
    ...
    time_twelve = time_twelve.replace(
        day=localtm.day, month=localtm.month, year=localtm.year
    )

    time_diff = localtm - time_twelve

    t1_time_seconds = time_diff.days * cn.seconds_in_day + time_diff.seconds
    ...
    lt_corr_degs = (t1_total_seconds / cn.seconds_in_day) * cn.full_circle
...
def get_ascendant(ramc_degs, latitude_degs, precsn_birth_degs, south_hemi):
    ...
    gaman_rads = math.atan(
        ((math.tan(latitude_rads) * math.sin(omega_rads)) / math.cos(ramc_rads))
        + math.cos(omega_rads) * math.tan(ramc_rads)
    )
...
def calculate_house(
    sine_value, ramc_degs, ramc_adder, pole_id, precsn_birth_degs, south_hemi
):
    """
    ...
    pole_elev_rads = math.atan(
        (1.0 / math.tan(omega_rads)) * math.sin(pole_id * pole_rads / 3.0)
    )

    oblasc_sub90_degs = find_diff_degs(oblasc_degs, 90)
    oblasc_sub90_rads = oblasc_sub90_degs * cn.rads_per_degree
    hlong_rads = math.atan(
        -1.0
        / (
            (
                math.tan(pole_elev_rads)
                * math.sin(omega_rads)
                / math.cos(oblasc_sub90_rads)
            )
...
def get_house_positions(
    ...
    if sine_value > 1:
        ...
        house_positions[10] = calculate_house(
            sine_value, ramc_degs, 30, 1, precsn_birth_degs, south_hemi
        )
        house_positions[11] = calculate_house(
            sine_value, ramc_degs, 60, 2, precsn_birth_degs, south_hemi
        )
        house_positions[1] = calculate_house(
            sine_value, ramc_degs, 120, 2, precsn_birth_degs, south_hemi
        )
        house_positions[2] = calculate_house(
            sine_value, ramc_degs, 150, 1, precsn_birth_degs, south_hemi
        )
...
def get_ecliptic_moon(true_moon_degs, rahu_degs):
    ...
    other_node_rads = math.atan(math.cos(theta_rads) * math.tan(nodal_dist_rads))
...
def get_moon_final_correction(mean_sun, mean_moon, sun_apse, moon_apse, rahu):
    ...
    a = find_diff_degs(mean_moon, mean_sun) * d2rad
    b = find_diff_degs(sun_apse, mean_sun) * d2rad
    c = find_diff_degs(moon_apse, mean_moon) * d2rad
    d = find_diff_degs(mean_sun, moon_apse) * d2rad
    e = find_diff_degs(mean_moon, rahu) * d2rad
    f = find_diff_degs(mean_sun, rahu) * d2rad

    corr_secs = (
        -155.0 * math.sin(2.0 * a + b)
        + 198.0 * math.sin(a + b - d)
        + 112.0 * math.sin(b - c)
        + 73.0 * math.sin(b + c)
        + 85.0 * math.sin(c + 2.0 * e)
        - 81.0 * math.sin(2.0 * f)
    )
...
def get_moon_params(input_params, sun_params):
    ...
    mean_rahu_degs = get_mean_longitude(
        epoch_days, rahu_rev_days, rahu_epoch, rahu=True
    )
    mean_ketu_degs = find_sum_degs([mean_rahu_degs, 180])

    net_corr_mins = get_net_correction(
        charam_degs, mandaphalam_secs, pranam_degs, long_degs, long_dirn
    )
    ...
    evection_mins = get_evection(
        mean_moon_corr_degs, mean_sun_corr_degs, mean_apse_corr_degs
    )
...
def get_helio_velocity(net_corr_mins, ma_degs, e):
    ...
    vel_mins = net_corr_mins * (
        1.0
        - 2.0 * e * math.cos(ma)
        + 5.0 * e * e * math.cos(2.0 * ma) / 2.0
        - (e * e * e / 4.0) * (13.0 * math.cos(3.0 * ma) - math.cos(ma))
    )
    vel_degs = vel_mins / 60.0
    return vel_degs


def get_longitude_along_ecliptic(
    epoch_node_degs, tlpd, inclination_rads, node_vel, years_elapsed
):
...
def get_geo_longitude(
    true_long_planet_degs,
    hvel_degs,
    inc_rads,
    node_planet_rads,
    rad_vec,
    ...
    if inferior == 0:  # Superior Planet (Outside earth orbit)
        ...
        tangle_mse_degs = (
            abs(tangle_mse_degs) + abs(angle_sme_degs) + abs(angle_mes_degs)
        )
...
def jupiter_correction(mean_long_degs, ist_time):
    ...
    adj_min = (
        20.8 * math.sin(t_rad * sj)
        - 1.3783 * math.sin(h_rad)
        + 3.4050 * math.sin(2.0 * h_rad)
        + 0.2830 * math.sin(3.0 * h_rad)
    )
...
def saturn_correction(mean_long_degs, ist_time):
    ...
    adj_min = (
        -48.7 * math.sin(t * sj_rad) + 7.0 * math.sin(x1_rad) + 10.85 * math.sin(x2_rad)
    )
...
def get_seven_planets(sun_params, moon_params):
    ...
    for name in planet_dict.keys():
        ...
        seven_planets[name] = get_planet_params(
            planet,
            ist_time,
            years_elapsed,
            epoch_days,
...
def get_planet_params(
    ...
    mean_long_degs = get_mean_longitude(
        epoch_days, planet.rev_days, planet.mean_long_at_epoch
    )
    ...
    apse_position_degs = get_apse_position_degs(mmae, apse_motion, years_elapsed)
    mean_anomaly_degs = get_mean_anomaly_degs(apse_position_degs, mean_long_degs)
    ...
    hvel_degs = get_helio_velocity(daily_motion, mean_anomaly_degs, eccentricity)
    radius_vec = get_radius_vector(
        apse_position_degs, true_long_degs, eccentricity, lsma
    )
    inc_rads = orbit_degs * cn.rads_per_degree
    planet_node_degs, true_long_degs = get_longitude_along_ecliptic(
        mmne, true_long_degs, inc_rads, node_motion, years_elapsed
    )
